...
Переключатель RU/EN в шапке сайта хранит выбор в cookie `locale` и
переводит витрину, авторизацию и личный кабинет (`src/i18n/dictionaries`).
Осознанно вне охвата:

- каталог (названия/описания товаров, категории) — это данные из БД, а не
  UI-строки, их перевод потребовал бы мультиязычной схемы контента;
- админ-панель — внутренний инструмент, остаётся на русском;
- сообщения валидации Zod и `<title>` вкладки — тоже на русском.

Из-за чтения cookie в корневом layout все страницы стали динамическими
(`ƒ` в выводе `next build`) — цена за то, что переключатель применяется
мгновенно без перезагрузки на другой домен/префикс.
